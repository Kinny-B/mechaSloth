...
The launch of the deepseek model, and the controversy and claims surrounding it, made me want to experiment with a localized model. The AI is also hosted locally inside the bot itself, this would remove any worry of data-harvesting on the user-end. Local-hosting the chat LLMs made the bot considerably slower, hence the name mechaSloth.
In future iterations would like to: Try a different localized chat LLM(not happy with deepseek performance). Upgrade hardware(new board, proximity sensors for better obstacle avoidance)
...
Mechanized sloth(Small Linguistics Optical Translating Helper). Powered by 'ollama deepseek-r1-1.5B LLM' distilled onto a raspberry-Pi-5. Able to to communicate via; Listening through microphones with 'VOSK Offline Speech Recognition API'. Speech using 'Piper Text-to-Speech' and speakers. Able to observe its environment via camera and 'Ultralytics YOLOv5 computer vision model'. Able to move itself via DC bi-directional motors. 
